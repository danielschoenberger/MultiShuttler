...
class ProcessingZone:
    def __init__(self, name, edge_idc):
        ...
        logging.debug(
            f"Initialized ProcessingZone with name: {name} and edge_idc: {edge_idc}"
        )


def preprocess(graph, priority_queue):
    logging.debug("Entered preprocess function")
    ...
    while sum(need_rotate) < len(priority_queue):
        for i, rotate_chain in enumerate(priority_queue):
            ...
            if (
                ...
                and next_edge not in state_edges_idc
                ...
                need_rotate[i] = True
    logging.debug("Exiting preprocess function")
...
def get_partitioned_priority_queues(graph, priority_queue, partition):
    logging.debug("Entered get_partitioned_priority_queues function")
    ...
    logging.debug(f"Partitioned priority queues: {partitioned_priority_queues}")
    return partitioned_priority_queues


def create_move_list(graph, partitioned_priority_queue, pz):
    logging.debug(f"Entered create_move_list function for processing zone: {pz.name}")
    ...
    logging.debug(f"Move list created: {move_list}")
    return move_list


def create_cycles_for_moves(graph, move_list, pz):
    logging.debug(
        f"Entered create_cycles_for_moves function for processing zone: {pz.name}"
    )
    all_cycles = {}
    ...
    logging.debug(f"All cycles for moves: {all_cycles}")
    return all_cycles


def find_conflict_cycle_idxs(graph, cycles_dict):
    logging.debug("Entered find_conflict_cycle_idxs function")
    ...
    logging.debug(f"Conflict cycle indexes found: {junction_shared_pairs}")
    return junction_shared_pairs


def find_movable_cycles(graph, all_cycles, priority_queue):
    logging.debug("Entered find_movable_cycles function")
    ...
    logging.debug(f"Movable cycles found: {free_cycle_seq_idxs}")
    return free_cycle_seq_idxs


def rotate(graph, ion, cycle_idcs):
    logging.debug(f"Entered rotate function for ion: {ion}")
...
def rotate_free_cycles(graph, all_cycles, free_cycles_idxs):
    ...
    logging.debug("Rotation of free cycles completed")
